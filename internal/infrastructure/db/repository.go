...
func (repository *Repository) CreateGameWithInitialRoute(ctx context.Context, game domain.Game, route domain.Route) (created *domain.Game, err error) {
	...
	var gameID string
	err = tx.QueryRowContext(ctx, `INSERT INTO "Game" (title, publisher, imagePath, exePath, saveFolderPath, localSaveHash, localSaveHashUpdatedAt, totalPlayTime, lastPlayed, clearedAt, playStatus, currentRouteId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id`, game.Title, game.Publisher, game.ImagePath, game.ExePath, game.SaveFolderPath, game.LocalSaveHash, game.LocalSaveHashUpdatedAt, game.TotalPlayTime, game.LastPlayed, game.ClearedAt, game.PlayStatus, game.CurrentRouteID).Scan(&gameID)
	if err != nil {
		return nil, err
	}
	if _, err = tx.ExecContext(ctx, `INSERT INTO "Route" (name, "order", gameId) VALUES (?, ?, ?)`, route.Name, route.Order, gameID); err != nil {
		return nil, err
	}
...
// DeleteGame は既存呼び出し向けにゲーム削除だけを行う。
func (repository *Repository) DeleteGame(ctx context.Context, gameID string) error {
	_, err := repository.connection.ExecContext(ctx, `DELETE FROM "Game" WHERE id = ?`, gameID)
	return err
}
...
func (repository *Repository) SetLocalSyncHead(ctx context.Context, gameID, hash string) error {
	_, err := repository.connection.ExecContext(ctx, `UPDATE "Game" SET localSyncHead = ? WHERE id = ?`, hash, gameID)
	return err
}
