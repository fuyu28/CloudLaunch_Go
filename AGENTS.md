...
- 大きな定数オブジェクトや長い JSX の **セクション見出し**は構造案内として可。
  ただし「ヘッダー」「モーダル」などラベルだけの連打は避ける
- **ファイル説明は what で OK、かつ本番ファイルでは必須:**
  - Go: `package` 直前のコメント（build tag の直後）。ファイルの役割を 1〜数文
  - Frontend: 先頭の `/** @fileoverview ... */`（タイトル + 1〜2文。`主な機能` 箇条書きや
    使用例コードは書かない）
  - 対象は本番 `.go` / `.ts` / `.tsx`（`*_test.go`・`*.test.ts(x)`・生成物は除外可）
  - 各 exported 記号への godoc / JSDoc 網羅は必須にしない
  - 型プロパティや atom 名の言い換え JSDoc（`/** タイトル */ title` や
    `LocalStorageに自動保存` の連打）は書かない。非自明な制約があるときだけ残す
  - フロントはファイル間でコメント量を揃えすぎないことより、**方針に沿った密度**を優先する。
    厚いのは複雑処理の説明、薄いのは単純 UI。自明 JSDoc で厚くしない
