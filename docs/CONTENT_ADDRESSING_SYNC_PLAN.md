...
```
games/{gameId}/objects/{sha256}   ← 全ブロブ（JSON・画像・セーブファイル）immutable
...
```
PutBlob(ctx, client, bucket, gameId, hash, data) error   // 既存なら skip
GetBlob(ctx, client, bucket, gameId, hash) ([]byte, error)
BlobExists(ctx, client, bucket, gameId, hash) (bool, error)
```

S3 キー: `games/{gameId}/objects/{hash}`
