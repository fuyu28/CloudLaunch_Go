...
| カラム | 型 | 意味 |
|---|---|---|
| `Game.localSyncHead` | TEXT | 最後に sync したときの**コンテンツ fingerprint**（共通祖先） |
| `Settings.device_name` | TEXT | このPCの表示名（`os.Hostname()` で初期化、ユーザー変更可） |

#### ⚠️ 2種類のハッシュが併存する

本システムには**目的の異なる2つのハッシュ空間**がある。混同しないこと。

| 呼称 | 計算対象 | 保存先 | 用途 |
|---|---|---|---|
| **MetaSnapshot hash**（`metaHash`） | MetaSnapshot 全体（`deviceName`・`createdAt` を含む） | S3 `games/{gameId}/HEAD` と `commits/{sha}` のキー | コミットオブジェクトの同一性・参照 |
| **コンテンツ fingerprint** | `{game.json hash, sessions.json hash, saves hash}` のみ（`deviceName`・`createdAt` を**除外**） | SQLite `Game.localSyncHead` | 同期状態判定の3値比較 |

判定に fingerprint を使う理由：`deviceName`・`createdAt` はデバイス・時刻ごとに必ず変わるため、`metaHash` をそのまま比較すると「同じ内容を別PCが push しただけ」でも常に差分に見えてしまう。**内容部分だけ**を取り出した fingerprint で比較することで、デバイス／時刻に依存しない「実体が同じか」の判定が成立する。

実装：`contentFingerprint()`（`internal/services/content_sync_service.go`）。

> このため `Status` / `Push` / `ResolveConflict` の3値比較（後述）は、すべて **fingerprint 同士**の比較である。`HEAD` に書かれた `metaHash` を直接 `localSyncHead` と比較してはならない。
...
いずれも**コンテンツ fingerprint**（前述）として比較する。

| 値 | 取得元 | 意味 |
|---|---|---|
| `currentLocalHash` | 現在のファイルをハッシュ → fingerprint 化 | 今のローカルの状態 |
| `localSyncHead` | SQLite（保存済み fingerprint） | 最後に sync したときの状態（共通祖先） |
| `remoteContentHash` | S3 `HEAD` → commit を取得 → fingerprint 化 | 今のリモートの状態 |
...
**Step 2:** `remoteHead` が存在する場合、`localSyncHead` が未設定（NULL）なら **`PullNeeded`**。
ベースライン（共通祖先）が無く3値比較ができないため、リモートを正とする。

**Step 3:** `localSyncHead` が存在する場合は三値の fingerprint 比較。

| currentLocalHash | remoteContentHash | 状態 |
|---|---|---|
| == localSyncHead | == localSyncHead | `InSync` |
| != localSyncHead | == localSyncHead | `PushNeeded`（ローカルのみ変更） |
| == localSyncHead | != localSyncHead | `PullNeeded`（リモートのみ変更） |
| != localSyncHead | != localSyncHead | `Conflict`（両方が独立して変更） |
...
type contentBlobStore interface {
    ...
    listGameIDs(ctx) ([]string, error)  // games/ 配下の HEAD からゲームIDを列挙
}
...
func (s *ContentSyncService) LoadCloudMetadata(ctx) ([]CloudGameInfo, error)  // クラウド上の全ゲームのメタ情報を復元
```

テストは `internal/services/content_sync_service_test.go`（Push / Pull / Status / ResolveConflict / DeleteFromCloud を網羅）。

**Status の流れ**
1. `remoteHead` を S3 から取得 → 空なら `NeverSynced` を返す
2. `remoteHead` の commit を取得し `remoteContentHash`（fingerprint）を算出
3. ローカルファイルをハッシュして `currentLocalHash`（fingerprint）を算出
4. SQLite から `localSyncHead`（fingerprint）を取得 → 未設定なら `PullNeeded`
5. 三値の fingerprint 比較で状態を決定
