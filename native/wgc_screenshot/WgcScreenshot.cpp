...
static HRESULT CreateD3DDevice(com_ptr<ID3D11Device>& device, com_ptr<ID3D11DeviceContext>& context) {
...
static HRESULT CreateDirect3DDeviceFromDXGI(const com_ptr<ID3D11Device>& device, IDirect3DDevice& outDevice) {
  ...
  outDevice = inspectable.as<IDirect3DDevice>();
  return S_OK;
}

static HRESULT SavePngFromTexture(
...
extern "C" __declspec(dllexport) HRESULT CaptureWindowToPngFile(HWND hwnd, const wchar_t* path) {
  if (!hwnd || !path) {
    return E_INVALIDARG;
  }
  ...
  hr = winrt::get_activation_factory<GraphicsCaptureItem, IGraphicsCaptureItemInterop>().as(interop);
  if (FAILED(hr)) {
    return hr;
  }

  GraphicsCaptureItem item{nullptr};
  hr = interop->CreateForWindow(hwnd, winrt::guid_of<GraphicsCaptureItem>(), reinterpret_cast<void**>(winrt::put_abi(item)));
  if (FAILED(hr)) {
    return hr;
  }
  ...
  hr = GetDXGIInterfaceFromObject(winrt::get_abi(surface), IID_PPV_ARGS(texture.put()));
  if (FAILED(hr)) {
    return hr;
  }
